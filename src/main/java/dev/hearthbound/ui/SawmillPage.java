...
public class SawmillPage extends InteractiveCustomUIPage<DialogEventData> {
    ...
    public SawmillPage(PlayerRef player, Ref<EntityStore> playerRef, World world,
    ...
    private void populateConstructionTab(UICommandBuilder b, BuildingRecord record) {
        boolean isBuilding = BuildingSystem.get().isBuilding()
                && record != null
                && BuildingType.SAWMILL.equals(BuildingSystem.get().getActiveRecord() != null
                        ? BuildingSystem.get().getActiveRecord().getType() : "");

        Map<String, Integer> required = BuildingSystem.getRequiredResources(BuildingType.SAWMILL);
        ...
        switch (action) {
            ...
            case "deposit" -> {
                ...
                Map<String, Integer> required = BuildingSystem.get().isBuilding()
                        ...
                        : BuildingSystem.getRequiredResources(BuildingType.SAWMILL);
                if (required == null) required = BuildingSystem.getRequiredResources(BuildingType.SAWMILL);
    ...
    private BuildingRecord findRecord(Store<EntityStore> store) {
        ...
        for (BuildingRecord b : village.getBuildings()) {
            if (BuildingType.SAWMILL.equals(b.getType())
                    && b.getPosX() == anchorX
                    && b.getPosY() == anchorY
                    && b.getPosZ() == anchorZ) {
                return b;
