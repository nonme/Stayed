...
/**
 * Woodcutter's Hut management UI — opened on F-key on the Woodcutters anchor block.
 */
public class WoodcuttersPage extends InteractiveCustomUIPage<DialogEventData> {

    private static final dev.hearthbound.util.log.Log LOG =
            dev.hearthbound.util.log.Log.get("ui.woodcutters");
    ...
    public WoodcuttersPage(PlayerRef player, Ref<EntityStore> playerRef, World world,
    ...
    private void showConfirmPhase(UICommandBuilder b) {
        ...
        b.set("#ConfirmInfo.Text",
                "A preview of the hut is shown.\n" +
                "Place the woodcutters block where you want it inside the building, " +
    ...
    private void populateInfoTab(UICommandBuilder b, Store<EntityStore> store, BuildingRecord record) {
        if (record == null || !record.isCompleted()) {
            ...
            b.set("#BuildingStatusDetail.Text", "Build the Woodcutter's Hut first.");
    ...
    private void populateConstructionTab(UICommandBuilder b, BuildingRecord record) {
        boolean isBuilding = BuildingSystem.get().isBuilding()
                && record != null
                && BuildingType.WOODCUTTERS_HUT.equals(BuildingSystem.get().getActiveRecord() != null
                        ? BuildingSystem.get().getActiveRecord().getType() : "");

        Map<String, Integer> required = BuildingSystem.getRequiredResources(BuildingType.WOODCUTTERS_HUT);
    ...
    private void applyConstructionState(UICommandBuilder b, boolean isBuilding,
        ...
        if (isCompleted) {
            b.set("#ConstructionStatus.Text",
                    "The Woodcutter's Hut stands complete. Upgrades coming in a future update.");
    ...
    public void handleDataEvent(Ref<EntityStore> ref, Store<EntityStore> store, DialogEventData data) {
        ...
        switch (action) {
            ...
            case "confirm" -> {
                ...
                BuildingRecord record = new BuildingRecord(BuildingType.WOODCUTTERS_HUT, anchorX, anchorY, anchorZ);
            ...
            case "deposit" -> {
                ...
                Map<String, Integer> required = BuildingSystem.get().isBuilding()
                        ...
                        : BuildingSystem.getRequiredResources(BuildingType.WOODCUTTERS_HUT);
                if (required == null) required = BuildingSystem.getRequiredResources(BuildingType.WOODCUTTERS_HUT);
    ...
    private BuildingRecord findRecord(Store<EntityStore> store) {
        ...
        for (BuildingRecord b : village.getBuildings()) {
            if (BuildingType.WOODCUTTERS_HUT.equals(b.getType())
                    && b.getPosX() == anchorX
                    && b.getPosY() == anchorY
                    && b.getPosZ() == anchorZ) {
                return b;
