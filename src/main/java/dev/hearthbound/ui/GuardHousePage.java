...
/**
 * Guard House management UI — opened when the player presses F on the Target Dummy anchor block.
 */
public class GuardHousePage extends InteractiveCustomUIPage<DialogEventData> {

    private static final Logger LOGGER = Logger.getLogger(GuardHousePage.class.getName());
    ...
    public GuardHousePage(PlayerRef player, Ref<EntityStore> playerRef, World world,
    ...
    private void showConfirmPhase(UICommandBuilder b) {
        ...
        b.set("#ConfirmInfo.Text",
                "A preview of the guard house is shown.\n" +
    ...
    private void populateInfoTab(UICommandBuilder b, Store<EntityStore> store, BuildingRecord record) {
        if (record == null || !record.isCompleted()) {
            ...
            b.set("#BuildingStatusDetail.Text", "Build the guard house first.");
        ...
        if (worker != null) {
            ...
            b.set("#BuildingStatusDetail.Text", "Patrolling.");
        } else {
    ...
    private void populateConstructionTab(UICommandBuilder b, BuildingRecord record) {
        boolean isBuilding = BuildingSystem.get().isBuilding()
                && record != null
                && BuildingType.GUARD_HOUSE.equals(BuildingSystem.get().getActiveRecord() != null
                        ? BuildingSystem.get().getActiveRecord().getType() : "");

        Map<String, Integer> required = BuildingSystem.getRequiredResources(BuildingType.GUARD_HOUSE);
    ...
    private void applyConstructionState(UICommandBuilder b, boolean isBuilding,
        ...
        if (isCompleted) {
            b.set("#ConstructionStatus.Text",
                    "The Guard House stands complete. Upgrades coming in a future update.");
    ...
    public void handleDataEvent(Ref<EntityStore> ref, Store<EntityStore> store, DialogEventData data) {
        ...
        switch (action) {
            ...
            case "confirm" -> {
                ...
                BuildingRecord record = new BuildingRecord(BuildingType.GUARD_HOUSE, anchorX, anchorY, anchorZ);
            ...
            case "deposit" -> {
                ...
                Map<String, Integer> required = BuildingSystem.get().isBuilding()
                        ...
                        : BuildingSystem.getRequiredResources(BuildingType.GUARD_HOUSE);
                if (required == null) required = BuildingSystem.getRequiredResources(BuildingType.GUARD_HOUSE);
    ...
    private BuildingRecord findRecord(Store<EntityStore> store) {
        ...
        for (BuildingRecord b : village.getBuildings()) {
            if (BuildingType.GUARD_HOUSE.equals(b.getType())
                    && b.getPosX() == anchorX
                    && b.getPosY() == anchorY
                    && b.getPosZ() == anchorZ) {
                return b;
