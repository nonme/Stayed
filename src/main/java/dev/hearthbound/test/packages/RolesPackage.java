...
/**
 * Stage 2 — roles. Validates the durable role-change path used by
 * ElfSage.respawnAs / RescueDialogPage.
 *
 * <ul>
...
public final class RolesPackage {
    ...
    private static TestCase buildRoleBasic() {
        return TestCase.of("role_basic",
                ...
                new ChangeRoleStep(r -> VILLAGER.equals(r.baseRoleName()), FARMER),
                new WaitStep(2_000),
                new AssertIdentityRoleStep(),
                new AuditStep(true, "post-to-farmer"),
                new ChangeRoleStep(r -> FARMER.equals(r.baseRoleName()), VILLAGER),
                new WaitStep(2_000),
                new AssertIdentityRoleStep(),
    ...
    private static TestCase buildRoleMass() {
        ...
        for (int i = 0; i < 3; i++) {
            steps.add(new ChangeRoleStep(r -> VILLAGER.equals(r.baseRoleName()), FARMER));
            steps.add(new WaitStep(1_000));
            steps.add(new AssertIdentityRoleStep());
            steps.add(new AuditStep(true, "to-farmer-" + i));
            steps.add(new ChangeRoleStep(r -> FARMER.equals(r.baseRoleName()), LUMBERJACK));
            steps.add(new WaitStep(1_000));
            steps.add(new AssertIdentityRoleStep());
            steps.add(new AuditStep(true, "to-lumber-" + i));
            steps.add(new ChangeRoleStep(r -> LUMBERJACK.equals(r.baseRoleName()), MINER));
            steps.add(new WaitStep(1_000));
            steps.add(new AssertIdentityRoleStep());
            steps.add(new AuditStep(true, "to-miner-" + i));
            steps.add(new ChangeRoleStep(r -> MINER.equals(r.baseRoleName()), VILLAGER));
            steps.add(new WaitStep(1_000));
            steps.add(new AssertIdentityRoleStep());
    ...
    private static TestCase buildRoleRescueToFollower() {
        ...
        return TestCase.of("role_rescue_to_follower",
                ...
                new ChangeRoleStep(r -> VILLAGER.equals(r.baseRoleName()), RESCUE_TRAP),
                new WaitStep(2_000),
                new AssertIdentityRoleStep(),
                new AuditStep(true, "post-to-trapped"),
                new ChangeRoleStep(r -> RESCUE_TRAP.equals(r.baseRoleName()), RESCUE_FOLLOW),
                new WaitStep(2_000),
                new AssertIdentityRoleStep(),
                new AuditStep(true, "post-to-follower"),
                new ChangeRoleStep(r -> RESCUE_FOLLOW.equals(r.baseRoleName()), VILLAGER),
                new WaitStep(2_000),
                new AssertIdentityRoleStep(),
                new AuditStep(true, "post-back-to-villager"))
                .withTeardown(
