...
/**
 * Tavern management UI — opened when the player presses F on the Tavern anchor block.
 */
public class TavernPage extends InteractiveCustomUIPage<DialogEventData> {

    private static final dev.hearthbound.util.log.Log LOG =
            dev.hearthbound.util.log.Log.get("ui.tavern");
    ...
    public TavernPage(PlayerRef player, Ref<EntityStore> playerRef, World world,
    ...
    private void showConfirmPhase(UICommandBuilder b) {
        ...
        b.set("#ConfirmInfo.Text",
                "A preview of the tavern is shown.\n" +
                "Place the cooking bench where you want it inside the building, " +
    ...
    private void populateInfoTab(UICommandBuilder b, Store<EntityStore> store, BuildingRecord record) {
        if (record == null || !record.isCompleted()) {
            ...
            b.set("#BuildingStatusDetail.Text", "Build the tavern first.");
        ...
        if (worker != null) {
            ...
            b.set("#BuildingStatusDetail.Text", "Serving food and drinks to villagers.");
        } else {
    ...
    private void populateConstructionTab(UICommandBuilder b, BuildingRecord record) {
        boolean isBuilding = BuildingSystem.get().isBuilding()
                && record != null
                && BuildingType.TAVERN.equals(BuildingSystem.get().getActiveRecord() != null
                        ? BuildingSystem.get().getActiveRecord().getType() : "");

        Map<String, Integer> required = BuildingSystem.getRequiredResources(BuildingType.TAVERN);
    ...
    private void applyConstructionState(UICommandBuilder b, boolean isBuilding,
        ...
        if (isCompleted) {
            b.set("#ConstructionStatus.Text",
                    "The Tavern stands complete. Upgrades coming in a future update.");
    ...
    public void handleDataEvent(Ref<EntityStore> ref, Store<EntityStore> store, DialogEventData data) {
        ...
        switch (action) {
            ...
            case "confirm" -> {
                ...
                BuildingRecord record = new BuildingRecord(BuildingType.TAVERN, anchorX, anchorY, anchorZ);
            ...
            case "deposit" -> {
                ...
                Map<String, Integer> required = BuildingSystem.get().isBuilding()
                        ...
                        : BuildingSystem.getRequiredResources(BuildingType.TAVERN);
                if (required == null) required = BuildingSystem.getRequiredResources(BuildingType.TAVERN);
    ...
    private BuildingRecord findRecord(Store<EntityStore> store) {
        ...
        for (BuildingRecord b : village.getBuildings()) {
            if (BuildingType.TAVERN.equals(b.getType())
                    && b.getPosX() == anchorX
                    && b.getPosY() == anchorY
                    && b.getPosZ() == anchorZ) {
                return b;
