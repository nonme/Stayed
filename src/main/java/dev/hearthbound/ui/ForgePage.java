...
/**
 * Forge management UI — opened when the player presses F on the Forge anchor block.
 */
public class ForgePage extends InteractiveCustomUIPage<DialogEventData> {

    private static final Logger LOGGER = Logger.getLogger(ForgePage.class.getName());
    ...
    public ForgePage(PlayerRef player, Ref<EntityStore> playerRef, World world,
    ...
    private void showConfirmPhase(UICommandBuilder b) {
        ...
        b.set("#ConfirmInfo.Text",
                "A preview of the forge is shown.\n" +
                "Place the forge anvil where you want it inside the building, " +
    ...
    private void populateInfoTab(UICommandBuilder b, Store<EntityStore> store, BuildingRecord record) {
        if (record == null || !record.isCompleted()) {
            ...
            b.set("#BuildingStatusDetail.Text", "Build the forge first.");
        ...
        if (worker != null) {
            ...
            b.set("#BuildingStatusDetail.Text", "Smithing arms and tools.");
        } else {
    ...
    private void populateConstructionTab(UICommandBuilder b, BuildingRecord record) {
        boolean isBuilding = BuildingSystem.get().isBuilding()
                && record != null
                && BuildingType.FORGE.equals(BuildingSystem.get().getActiveRecord() != null
                        ? BuildingSystem.get().getActiveRecord().getType() : "");

        Map<String, Integer> required = BuildingSystem.getRequiredResources(BuildingType.FORGE);
    ...
    private void applyConstructionState(UICommandBuilder b, boolean isBuilding,
        ...
        if (isCompleted) {
            b.set("#ConstructionStatus.Text",
                    "The Forge stands complete. Upgrades coming in a future update.");
    ...
    public void handleDataEvent(Ref<EntityStore> ref, Store<EntityStore> store, DialogEventData data) {
        ...
        switch (action) {
            ...
            case "confirm" -> {
                ...
                BuildingRecord record = new BuildingRecord(BuildingType.FORGE, anchorX, anchorY, anchorZ);
            ...
            case "deposit" -> {
                ...
                Map<String, Integer> required = BuildingSystem.get().isBuilding()
                        ...
                        : BuildingSystem.getRequiredResources(BuildingType.FORGE);
                if (required == null) required = BuildingSystem.getRequiredResources(BuildingType.FORGE);
    ...
    private BuildingRecord findRecord(Store<EntityStore> store) {
        ...
        for (BuildingRecord b : village.getBuildings()) {
            if (BuildingType.FORGE.equals(b.getType())
                    && b.getPosX() == anchorX
                    && b.getPosY() == anchorY
                    && b.getPosZ() == anchorZ) {
                return b;
